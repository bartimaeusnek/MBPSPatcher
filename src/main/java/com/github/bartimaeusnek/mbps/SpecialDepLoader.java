...
/**
 * Taken from CodeChickenCore and modified
 *
 * For autodownloading stuff.
 * This is really unoriginal, mostly ripped off FML, credits to cpw.
 */
public class SpecialDepLoader implements IFMLLoadingPlugin, IFMLCallHook {
    private static ByteBuffer downloadBuffer = ByteBuffer.allocateDirect(1 << 24);
    private static final String owner = "SpecialDepLoader";
    private static SpecialDepLoadInst inst;
    ...
    public static class Dependency
    {
        ...
        public Dependency(String url, VersionedFile file, boolean coreLib, boolean noSub) {
            this.url = url;
            this.file = file;
    ...
    public static class SpecialDepLoadInst {
        ...
        private void addClasspath(String name) {
            try {
                ((LaunchClassLoader) SpecialDepLoader.class.getClassLoader()).addURL(new File(v_modsDir, name).toURI().toURL());
            } catch (MalformedURLException e) {
                throw new RuntimeException(e);
            }
        }
        private void addModsClasspath(String name) {
            try {
                ((LaunchClassLoader) SpecialDepLoader.class.getClassLoader()).addURL(new File(modsDir, name).toURI().toURL());
            } catch (MalformedURLException e) {
                throw new RuntimeException(e);
            }
        }
        ...
        private void deleteMod(File mod) {
            ...
            try {
                ClassLoader cl = SpecialDepLoader.class.getClassLoader();
        ...
        private void download(Dependency dep) {
            ...
            File libFile = !dep.noSub ? new File(v_modsDir, dep.file.filename) : new File(modsDir, dep.file.filename);
            try {
        ...
        private String checkExisting(Dependency dep) {
            for (File f : modsDir.listFiles()) {
                ...
                if (dep.noSub) {
                    ...
                    f.renameTo(new File(v_modsDir, f.getName()));
                }
            }
            ...
            for (File f : v_modsDir.listFiles()) {
                ...
                if (dep.noSub) {
                    f.renameTo(new File(modsDir, f.getName()));
                }
                return f.getName();//found dependency
            }
            return null;
        ...
        private void scanDepInfo(File file) {
            try {
                ...
                ZipEntry e = zip.getEntry("SpecialDependancies.info");
                if (e == null) e = zip.getEntry("SpecialDependancies.info");
                if (e != null)
                    loadJSon(zip.getInputStream(e));
                zip.close();
            } catch (Exception e) {
                System.err.println("Failed to load SpecialDependancies.info from " + file.getName() + " as JSON");
                e.printStackTrace();
            }
        }
        ...
        private void loadJson(JsonObject node) throws IOException {
            boolean obfuscated = ((LaunchClassLoader) SpecialDepLoader.class.getClassLoader())
                    .getClassBytes("net.minecraft.world.World") == null;

            String testClass = node.get("class").getAsString();
            if (SpecialDepLoader.class.getResource("/" + testClass.replace('.', '/') + ".class") != null)
                return;

            String repo = node.get("repo").getAsString();
            String filename = node.get("file").getAsString();

            boolean noSub = node.has("noSub") && node.get("noSub").getAsBoolean();
            ...
            addDep(new Dependency(repo, file, coreLib,noSub));
        }
